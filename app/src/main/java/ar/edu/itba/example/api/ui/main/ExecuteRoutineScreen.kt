...
fun ExecuteMainScreen(
    ...
    var cycleRepetitions by rememberSaveable { mutableStateOf(cyclesWithExercise[cycleIndex].cycle?.repetitions) }

    if(cyclesWithExercise[cycleIndex].exercises?.isEmpty() == true){
        if(cycleIndex + 1 >= cyclesWithExercise.size){
        ...
        var timeLeft by rememberSaveable {
            mutableStateOf(
                cyclesWithExercise[cycleIndex].exercises?.get(
                    exerciseIndex
                )?.duration
            )
        }
        ...
        Column(
            ...
            if (exerciseDetail) {
                ...
                if (cyclesWithExercise[cycleIndex].exercises?.get(exerciseIndex)?.exercise?.detail != cyclesWithExercise[cycleIndex].exercises?.get(
                        exerciseIndex
                    )?.exercise?.name && orientation == Configuration.ORIENTATION_PORTRAIT
                ) {
                ...
                if (orientation == Configuration.ORIENTATION_PORTRAIT || windowHeight >= 700.dp) {
                    ...
                    val exercisesToShow =
                        if (windowWidth <= 600.dp || windowHeight <= 480.dp) 1 else 3


                    while (j < cyclesWithExercise.size && total < exercisesToShow) {
                        OutlinedCard(
                            colors = CardDefaults.cardColors(
                                containerColor = MaterialTheme.colorScheme.surface
                            ),
                ...
                Button(
                    onClick = {
                        if (exerciseIndex + 1 < cyclesWithExercise[cycleIndex].exercises?.size!!) {
                            ...
                            cycleRepetitions = cycleRepetitions!! - 1
                            if (cycleRepetitions == 0) {
                                cycleIndex++
                                exerciseIndex = 0
                                cycleRepetitions = cyclesWithExercise[cycleIndex].cycle?.repetitions
                            } else {
                                exerciseIndex = 0
                            }
                        }
                        if (cyclesWithExercise[cycleIndex].exercises?.isEmpty() == true) {
                            ...
                            timeLeft =
                                cyclesWithExercise[cycleIndex].exercises?.get(exerciseIndex)?.duration
                            isPaused = true
                        }
                    },
                    modifier = Modifier.size(250.dp, 75.dp),
                ) {
                    Text(
            ...
            Button(
                ...
                Text(
                    text = if (exerciseDetail) stringResource(id = R.string.view_all_exercises) else stringResource(
                        id = R.string.view_detailed_exercise
                    ),
                    fontSize = 20.sp,
                    color = MaterialTheme.colorScheme.onPrimary
                )
